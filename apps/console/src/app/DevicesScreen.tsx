/**
 * The device screen — what the console is for, and until the sign-in work the whole of it.
 *
 * IT NO LONGER DECIDES WHETHER YOU ARE SIGNED IN. `App.tsx` is the gate now: it calls `whoami()`,
 * which is both the check and what recovers the CSRF token, and only renders this once that has
 * come back. So the `signed-out` branch that used to live in the device list — the one that told
 * people to go and sign in on the old console at `/` — is gone, along with the reason for it.
 *
 ...
 * nothing.
 *
 * Served at `/app/devices` alongside the old console at `/`, so neither blocks the other and a
 * cutover is one line in the API's allowlist.
 */
...
import {
  ApiError, getSession, listDevices, releaseSession, startSession,
  type Device, type SessionDetail,
} from './api.ts';
import { signedOut } from './routes.ts';
...
export function DevicesScreen() {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        // No `whoami` here any more — the gate in `App.tsx` has already made that call, which is
        // what both proved the session and recovered the CSRF token every mutation below needs.
        ...
        if (!live) return;
        // A 401 here means the cookie expired between the gate's check and this call. `signedOut`
        // sends the person back to sign in with this page remembered, rather than leaving them on a
        // device list that will 401 every button.
