...
| C2 | AI run engine | **Shipped** | #203 | 2026-09-26 on the farm (Groq `qwen/qwen3.8-27b`, Cuttlefish): Flash and Pro runs drove real devices end to end. `src/ai/agent.ts` (loop) + `runner.ts` (claims, per-run key, hub via `inject`); `/v1/ai/*`; migration 061 |
...
- **Verifying without a paid key (2026-09-26).** The owner wants C2–C10 exercised on the farm before
  ...
  AI runs". Caveat: a free model's token counts do NOT calibrate the price of the model we will sell on.
