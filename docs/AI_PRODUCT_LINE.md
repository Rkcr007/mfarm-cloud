...
| C5 | Console AI section | **Shipped** | #204, #216, #224 | Farm › AI testing (`#/ai`, `G E`): prompt, Flash/Pro, build, region; recent runs; budget meter; run page with step trajectory + the screen at each step, Stop, Run again, link to the session's recording & log. Prices only from `/v1/ai/pricing` **2026-09-26, explored on the farm by hand:** the run list could say "Loading…" for good (D49 — a person could not find the run they had just started); the run page had no visible way back, hid why a run stopped (a provider's daily cap showed as "could not be reached"), called a step that failed "Typed…", showed a model's tool-call markup, lost its screenshot off-screen, offered no diagnosis on a failed run, and "Run again" dropped the app build; Share said "Share this failure" on a passed run. All fixed in #216, each with a test that fails with it put back. **2026-09-27 redesign (#224):** Runs, New run and Saved tests are separate tabs, each with its own address; the list is one line per run with running ones pinned on top, and its search and filters live in the address, so Back returns to the same place and Newer/Older walk the runs. A run's page is titled by its task (the browser tab too), with one line of facts and the rest under Details; one line per step, and a failed step says why in plain words; the screenshot marks where the agent tapped (each step now records the screen size its coordinates are measured on — points on iOS, not the screenshot's pixels); recording and log open in a side panel; ←/→ step through screens, J/K move between runs, Esc closes then goes back; extra actions under More; a notice when a run you started finishes while you are elsewhere. **Verified on the farm 2026-09-27** (build `1ea7c9c`, headless Chrome signed in as the farm admin, 0 exceptions, 0 console errors, 0 failed requests): tabs and their addresses, one line per run, the filter kept in the address and Esc returning to it, Newer / Older and J / K in the list's order, the task as the tab title, Details collapsed, every failed step in words ("There was no text field ready to type into."), the tap outlined and dotted exactly on "Search Settings" on a real Android screenshot, the recording and log panel, More, and the finish notice arriving on Fleet with an Open that goes to the run. **Not verified on hardware:** `action.screen` on a new run — the one run started was refused by the model provider before its first step (D54); tests cover it. |
| C6 | Saved AI tests | **Shipped** | #205, #214 | 2026-09-26 on the farm (Groq `qwen/qwen3.8-27b`, Cuttlefish): a saved test ran on demand against `io.appium.android.apis@latest` and passed; its history shows each run. #214: a saved test with no region (what the console saves on a one-region farm) now resolves one per run. `ai_tests` (migration 062): name, prompt, mode, the app package it is about; Run = `<package>@latest`; last 10 verdicts on the row; archive keeps history **2026-09-27 (#224):** a saved test can be edited. The edit box starts from the whole task (`GET /v1/ai/tests/:id/prompt`, the twin of the run's read-back), and a task that still holds `••••` is refused at every door — New run, Save as test, Edit — so the mask can never be saved over a PIN and typed into the app. On the farm 2026-09-27: all three doors answer 400 with the reason and queue nothing, a refused edit leaves the task as it was, and Edit on a saved test with a hidden value fetched the whole task into the box. |
...
- **Verifying without a paid key (2026-09-26).** The owner wants C2–C10 exercised on the farm before
  ...
  AI runs". ****2026-09-27: a third ceiling, and this one refuses rather than waits — 1,000 OUTPUT tokens a minute (OTPM), and an uncapped request is assumed to want 1,748, so every call is refused (D54).** The day's real ceiling is 200,000 tokens (TPD)** — about 45 steps; one afternoon of
...
- **Named secrets — not started (2026-09-27).** Masking (D52) finds a secret by the label before it
  ...
  it deferred; it changes what is stored and what the model is told, so it gets its own ADR and PR.
