...
apps/api/          Fastify + Postgres. Allocation, leases, fencing, runs, artifacts,
                   the WebDriver hub, RLS tenancy, metrics. 63 migrations.
...
docs/adrs/         43 decisions, each with what was rejected and why (numbered to 0044;
                   there is no 0013).
```
...
**Fifty-three defects are recorded. Almost none were found by the test suite.** They were found by
...
2. `docs/adrs/` — 43 decisions (numbered to 0044; there is no 0013). Each says what was rejected
