...
|---|---|---|
| **Console (UI)** | **Working** — the full design package on the console at `/`: Fleet, catalogue, cockpit, bring-up, apps, runs, health, agents, team, settings. Both themes. Zero console exceptions across every surface. | Twenty-five defects have been found in it, all by USING it and **none by the 1441-test suite**. All are closed. |
| **Console at `/app`** (React) | **Two screens.** Sign-in (2026-09-06) and the live device view. It has its own session gate now, so `/app` no longer sends people to `/` to sign in and come back. | **Not at parity and not the front door.** No fleet, catalogue, apps, runs or health screens — `/` remains the console people use. Three of its defects were found by driving it in a browser and none by its 79 tests. |
