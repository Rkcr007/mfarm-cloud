...
| **API / control plane** | **Working** — allocation, leases, fencing, reset, quarantine and gated recovery, runs, outcomes, artifacts, RLS tenancy, metrics. 63 migrations. API keys are labelled, scoped, expiring and attributed (ADR-0034). | **Single instance only.** Rate limiting is in-memory, so a second API process silently multiplies every limit. |
...
| **AI runs (ADR-0043)** | **Built, off until configured.** `POST /v1/ai/runs` queues an English-described test; the runner drives a device through the hub (Flash / Pro), bills each step from `ai/pricing.ts`, and reports the verdict on the session. `mfarm mcp` lets a customer's own agent drive a device. Tracker: `docs/AI_PRODUCT_LINE.md`. | Off on the farm until `MFARM_AI_API_KEY` is set (any provider — `MFARM_AI_PROVIDER`); not yet run against a real device or a real model. No console section yet (C5). |
...
| **Sharing a failure** | **Built 2026-09-12 (ADR-0036, migration 051); widened 2026-09-14 (ADR-0040, migration 057).** A revocable, expiring link at `/s/<token>` shows ONE test result — its message, the screenshot captured for it, and the steps between the previous test and this one — to somebody with no account here. Its own page, not the console. Since ADR-0040 a link can also carry the session's **logcat** and **recording** (played from the failure); the console's dialog ticks both by default, the API defaults both off. | **Carrying the log is the owner's accepted risk:** nobody reads a logcat before sending it, and apps print tokens into it — the dialog says so beside the box. The recording shows every test on the session, not only this one. A link can outlive the evidence it points at, since artifacts go on their org's retention schedule. |
...
- **On the run screen, a session names its tests in its own row** (ADR-0041). Failed tests are named
  straight away — the run already carries them — and the `passed/total` count unfolds the rest in
  place, one request per row a person opens, so a nightly run of two hundred sessions costs nothing
  until somebody asks. Several rows can be open at once, which is the point: "which of these ran the
  OTP scenario" is a comparative question. Each failing test also carries its last runs and a plain
  verdict — newly broken, flaky, failing every time (ADR-0039).
- **On the session screen's Steps tab, a Tests card** lists everything that session reported, not only what
