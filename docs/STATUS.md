...
| **API / control plane** | **Working** — allocation, leases, fencing, reset, quarantine and gated recovery, runs, outcomes, artifacts, RLS tenancy, metrics. 55 migrations. API keys are labelled, scoped, expiring and attributed (ADR-0034). | **Single instance only.** Rate limiting is in-memory, so a second API process silently multiplies every limit. |
...
| **Infrastructure ops** | **Working, deployed 2026-09-13** (ADR-0038, migrations 053–055). `#/infra` — six sections, an overall health rollup with the evidence under every light, per-host gauges that age separately from the heartbeat, cost from a power ledger rather than from `up_since`, an append-only operations log with five filters, and an SSE stream. Drain and resume are live; every operation is idempotent and audited. Gated on `users.operator`, a **farm-wide** capability no API key can hold. | **Power is code-complete and off.** It needs an IAM role AND a change to `mfarm-cp`'s OAuth scopes, which requires stopping the control plane — `RUNBOOK.md` has the commands. Until then `capabilities.power` is false and the page says so. **Per-service restart** (Caddy, coturn, Appium, cvd) is not built: it needs the agent to learn a job kind, so those still need SSH. |
